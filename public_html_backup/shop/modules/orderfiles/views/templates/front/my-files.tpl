{capture name=path}<a href="{$link->getPageLink('my-account', true)}">{l s='My account' mod='orderfiles'}</a><span class="navigation-pipe">{$navigationPipe}</span>{l s='Files uploader' mod='orderfiles'}{/capture}
{if $psversion==5}{include file="$tpl_dir./breadcrumb.tpl"}{/if}
		...
		{foreach from=$orders key=id item=order}
			...
			{else}
				<tr class="first_item ">
					...
					<td class="history_detail"><form method="post" action="{$link->getModuleLink('orderfiles', 'filesmanager')}"><input type="hidden" name="oid" value="{$order.id_order}" /><input type="submit" class="button" value="{l s='Manage' mod='orderfiles'}"/></form></td>
				</tr>
			{/if}
		{/foreach}
	</tbody>
