<div class="header_user_info col-xs-12 col-sm-4 col-sm-offset-{4-$warehouse_vars.logo_width}">
	<p>{if $is_logged} <a href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}" title="{l s='View my customer account' mod='blockuserinfo'}" class="account" rel="nofollow"><i class="icon-user"></i> <span>{$cookie->customer_firstname} {$cookie->customer_lastname}</span></a> / 
...
<p onclick="fblogin_mypresta();"><span>{l s='' mod='fblogin'}</span></p>
</div>
{/if}
</p>
</div>
