...
function EasyPayPalDelete(post_ID, item_number)
{
    var ret = false;
...
if(!empty($content_items))
{
	...
	foreach($content_items as $item)
	{
		...
		echo '<td>'.$item['item_number'].'</td><td><span class="submit"><input type="submit" name="submit_content_delete" onclick="return EasyPayPalDelete('.'\''.$item['post_ID'].'\''.','.'\''.$item['item_number'].'\''.')" value="'. _('Delete &raquo;').'" /></span></td></tr>'; 
...
<hr/>
<h3>Export</h3>
<p><a href="<?php echo get_option('siteurl').'/wp-content/plugins/easypaypal/easypaypal_user_export.php'?>" target="_blank">Export User Information</a></p>
</div>
</form>
