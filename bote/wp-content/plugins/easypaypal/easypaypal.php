<?php
/*
Plugin Name: EasyPayPal
Plugin URI: http://www.voiceoftech.com/swhitley/index.php/?page_id=129 
Description: Users will be charged a fee through PayPal when they login to your WordPress site.  The 'Next Payment' date will be set for each user based on the number of days since their last payment.  After the 'Next Payment' date, they must make another payment through PayPal before they will be allowed to login to your site again.
Version: 3.5
Date: August 15, 2006
...
Modifications:
**************************************************************
05/22/2008 S. Whitley    Removed IPN Code & Added Return page.
**************************************************************
*/ 
...
//************************************************************
$PostPaymentPageTitle = get_option('easypaypal_post_payment_page');
$SubscriptionPaymentPageTitle = get_option('easypaypal_subscription_payment_page');
$CategoryPaymentPageTitle = get_option('easypaypal_category_payment_page');
...
add_action('admin_menu', 'EasyPayPal_Config_Page');

$epp_payment_email = get_option("easypaypal_payment_email");

if(strlen($epp_payment_email) > 0)
{
    add_action ('admin_head', 'EasyPayPal');
    add_action ('load-page-new.php','EasyPayPal_PostPayment');
    add_action ('load-post-new.php','EasyPayPal_PostPayment');
    add_filter('the_content', 'EasyPayPal_the_content');
    add_filter('the_content', 'EasyPayPal_the_custom_content');
    add_filter('the_content', 'EasyPayPalButton_the_content');
    add_action('edit_form_advanced', 'EasyPayPal_Editor');
    add_action('edit_page_form', 'EasyPayPal_Editor');
    add_action('publish_post', 'EasyPayPal_Publish');
}



/********************************************************************
* Function:  EasyPayPal_PostPayment()
...
function EasyPayPal_PostPayment()
{
    ...
    if(EasyPalPal_Content_Purchased($user_ID,$post_ID,''))
    {
	...
	if(!$edit && !empty($post_ID) && !EasyPayPal_BypassCheck())
	{
...
/********************************************************************
* Function:  EasyPayPal_Editor()
* Display instructions on editor page.
********************************************************************/
function EasyPayPal_Editor()
{
    echo '<h3><a href="javascript:void(0)" onclick="eppDiv=document.getElementById(\'easypaypal_editor\');eppDiv.style.display == \'none\' ? eppDiv.style.display = \'block\' :eppDiv.style.display = \'none\';">EasyPayPal Instructions (Show/Hide)</a></h3><br/>';
    echo '<div id="easypaypal_editor" style="display:none;border:solid 1px #CCCCCC;background-color:#F1F1F1;padding:10px;">';
...
/********************************************************************
* Function:  EasyPayPal_Publish()
* Decrement credits when post is published.
********************************************************************/
function EasyPayPal_Publish($post_ID)
{
    global $wpdb, $user_ID, $PostPaymentPageTitle;
...
/********************************************************************
* Function:  EasyPayPal_the_content()
* Hide text if not a user.
********************************************************************/
function EasyPayPal_the_content($text) {
   ...
   {
       $epp_more_login = get_option('easypaypal_more_login');
   }
    
   if(! is_user_logged_in())
   {
       $text = EasyPayPal_HideText($text, $tag_pattern);
   }
   else
   {
      if (EasyPayPal_Expired($user_ID)) {
         $text = EasyPayPal_HideText($text, $tag_pattern);
      }
      else
      {
...
/********************************************************************
* Function:  EasyPayPal_the_custom_content()
* Hide text based on categories.
********************************************************************/
function EasyPayPal_the_custom_content($text) {
    ...
    if(!EasyPayPal_BypassCheck())
    {
		...
		if(strlen($category_id) > 0 || strlen(get_post_meta($post_ID, "cat", true)) > 0)
	    {
	        ...
	        else
	        {
	            ...
	            {
	            	//Loop through EasyPayPal Categories
		           if(is_array($cats))
                              {
                                foreach($cats as $cat)
		              {
	            ...
	            if(!EasyPalPal_Content_Purchased($user_ID,$cat_post_ID,$category_id))
	            {
		            $text = $epp_more_login;
	            }
	        }
...
function EasyPayPalButton_the_content($text) {
     ...
     foreach($matches as $match)
     {
        foreach($match as $repl)
        {
	        ...
	        if(count($buttonVars) >= 6)
	        {
                 ...
                 if(!EasyPayPal_BypassCheck() && !EasyPalPal_Content_Purchased($user_ID,$post_ID,$buttonVars[1]))
                 {
                 	$returnURL = '';
  	             ...
  	             $pay_text = EasyPayPal_Form($buttonVars, $returnURL);
                 }
...
function EasyPalPal_Content_Purchased($user_ID, $post_ID, $epp_content_item_number)
{
      $content_items = get_usermeta($user_ID,'eppItems'); 
      is_array($content_items) ? '' : $content_items = array();
      $found = false;
...
function EasyPayPal_HideText($text, $tag_pattern){
...
function EasyPayPal_Config_Page() {
	global $wpdb;
	global $epp_menu;
...
function EasyPayPal_Expired($user_ID) {
    ...
    $ret = false;
    $result = EasyPayPal_PaymentDateGet($user_ID);
    
    if (!EasyPayPal_BypassCheck() && (strlen($result) == 0 || $result < date("Y-m-d"))) {
        $ret = true;
    }
...
function EasyPayPal_PaymentDateGet($user_ID) {

  global $epp_trial;
  
  empty($epp_trial)  ? $epp_trial = get_option("easypaypal_trial"): '';  
  ...
  {
     $trial_ends = EasyPayPal_DateAdd($epp_trial);
...
function EasyPayPal() {
  ...
  if (EasyPayPal_Expired($user_ID)) {
     $text = EasyPayPal_PageGet();
     if(strlen($text) > 0)
     {
        $epp_user_msg = EasyPayPalButton_the_content($text);
     }
     else
     {
        $formVars = array();
        echo EasyPayPal_Form($formVars);
     ...
     EasyPayPal_Alert($epp_user_msg); 
     die(); 
  }

}

function EasyPayPal_Form($formVars, $returnURL='')
{
  /********************************************************************
   * EasyPayPal variables. 
   * 
  ...
  if(count($formVars) >= 5)
  {
   ...
   empty($epp_payment_email) ? $epp_payment_email = get_option("easypaypal_payment_email"):'';
   if(strlen($returnURL) > 0)
   {
   		$epp_return_URL = $returnURL;
   }
   empty($epp_return_URL) ? $epp_return_URL = EasyPayPal_selfURL():'';
   empty($epp_item_name) ? $epp_item_name = get_option("easypaypal_item_name"):'';
   empty($epp_item_number) ? $epp_item_number = get_option("easypaypal_item_number"):'';
   empty($epp_amount) ? $epp_amount = get_option("easypaypal_amount"):'';
   empty($epp_currency_code) ? $epp_currency_code = get_option("easypaypal_currency_code"):'';
   empty($epp_days_to_add) ? $epp_days_to_add = get_option("easypaypal_days_to_add"):'';
   empty($epp_user_msg)  ? $epp_user_msg = get_option("easypaypal_user_msg"):'';
   empty($epp_trial)  ? $epp_trial = get_option("easypaypal_trial"): '';
   ...
   ********************************************************************/
   empty($epp_test) ? $epp_test = get_option("easypaypal_test"): '';
   if($epp_test == "checked")
   {
...
//     $epp_period = EasyPayPal_Period($epp_days_to_add);
...
//     $epp_days_to_add = 9999;
  ...
  $epp_form .= '<input type="hidden" name="return" value="'.$plugin_url.'/easypaypal_return.php">';
  ...
  $epp_auth_code= get_option("easypaypal_auth_code");
...
function EasyPayPal_Period($epp_days_to_add)
{
    $t3 = 'D';
    $p3 = $epp_days_to_add;
...
function EasyPayPal_BypassCheck()
{
  global $epp_bypass;
  global $user_login;
   ...
   ********************************************************************/
   empty($epp_bypass) ? $epp_bypass = get_option("easypaypal_bypass"):'';
...
//DateAdd
function EasyPayPal_DateAdd($v,$d=null ,$f="Y-m-d"){ 
  $d=($d?$d:date("Y-m-d")); 
  return date($f,strtotime($v." days",strtotime($d))); 
}

function EasyPayPal_selfURL() {
	$s = empty($_SERVER["HTTPS"]) ? ''
		: ($_SERVER["HTTPS"] == "on") ? "s"
		: "";
	$protocol = EasyPayPal_strleft(strtolower($_SERVER["SERVER_PROTOCOL"]), "/").$s;
	$port = ($_SERVER["SERVER_PORT"] == "80") ? ""
		: (":".$_SERVER["SERVER_PORT"]);
	return $protocol."://".$_SERVER['SERVER_NAME'].$port.$_SERVER['REQUEST_URI'];
}
function EasyPayPal_strleft($s1, $s2) {
